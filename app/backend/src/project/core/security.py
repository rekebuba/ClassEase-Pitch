...
def create_access_token(
    *,
    ...
    session_scope: SessionScope = SessionScope.SCHOOL,
    ...
    to_encode: dict[str, Any] = {
        ...
        "scope": session_scope.value,
        "jti": str(uuid.uuid4()),  # Unique identifier for token
        "iat": datetime.now(timezone.utc),  # Issued at time
    }
    if session_scope == SessionScope.SCHOOL:
        if role is None:
            raise ValueError("School-scoped access tokens require a school role.")
        to_encode.update(
            {
                "role": role.value,
                "school_id": school_id,
...
async def is_token_blacklisted(token: str, session: AsyncSession) -> bool:
    try:
        payload: dict[str, Any] = jwt.decode(
            ...
            settings.SECRET_KEY.get_secret_value(),
            algorithms=[ALGORITHM],
            options={"verify_exp": False},  # We still want to check blacklist even if expired
        )
        jti = payload.get("jti")
