...
class AcademicTermFactory(TypedFactory[AcademicTerm]):
    ...
    registration_start = factory.LazyAttribute(lambda o: o.start_date - timedelta(days=14))
...
class StudentFactory(TypedFactory[Student]):
    ...
    transportation = factory.LazyFunction(lambda: random.choice(["bus", "walk", "parent"]))
...
class SubjectOfferingFactory(TypedFactory[SubjectOffering]):
    class Meta:
    ...
    grade_id = factory.LazyAttribute(lambda o: o.grade_obj.id)
    stream_id = factory.LazyAttribute(lambda o: o.stream_obj.id if o.stream_obj else None)
...
class StudentTermRecordFactory(TypedFactory[StudentTermRecord]):
    ...
    grade_obj = factory.SubFactory(GradeFactory)
    section_obj = factory.SubFactory(SectionFactory, grade_obj=factory.SelfAttribute("..grade_obj"))
    stream_obj = factory.SubFactory(StreamFactory, grade_obj=factory.SelfAttribute("..grade_obj"))
...
class AssessmentFactory(TypedFactory[Assessment]):
    class Meta:
        model = Assessment
        exclude = ("student_obj", "student_term_record_obj", "subject_offering_obj")
    ...
    subject_offering_obj = factory.SubFactory(
        SubjectOfferingFactory,
    )

    student_id = factory.LazyAttribute(lambda o: o.student_obj.id)
    student_term_record_id = factory.LazyAttribute(lambda o: o.student_term_record_obj.id)
    subject_offering_id = factory.LazyAttribute(lambda o: o.subject_offering_obj.id)
...
class SubjectYearlyAverageFactory(TypedFactory[SubjectYearlyAverage]):
    class Meta:
        ...
        exclude = ("student_obj", "subject_offering_obj", "student_year_record_obj")
...
class AuditLogFactory(TypedFactory[AuditLog]):
    ...
    school_id = factory.LazyAttribute(lambda o: o.school_obj.id if o.school_obj else None)
    user_id = factory.LazyAttribute(lambda o: o.user_obj.id if o.user_obj else None)
    membership_id = factory.LazyAttribute(lambda o: o.membership_obj.id if o.membership_obj else None)
    auth_session_id = factory.LazyAttribute(lambda o: o.auth_session_obj.id if o.auth_session_obj else None)
...
class TransferRequestFactory(TypedFactory[TransferRequest]):
    ...
    record_scope = factory.LazyFunction(lambda: random.choice(["student", "employee", "full"]))
