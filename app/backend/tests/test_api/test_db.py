...
from tests.test_api.factories import UserFactory


def test_db_grade_count(client: FlaskClient, db_session):
    # Query the database
...
def test_db_grade_values(client: FlaskClient, db_session):
    # Query the database
...
def test_db_subject_count(client: FlaskClient, db_session):
    # Query the database
    subject = db_session.query(Subject).count()
    assert subject > 0


def test_db_year_count(client: FlaskClient, db_session):
    # Query the database
    year = db_session.query(Year).count()
    assert year > 0


def test_db_table_count(client: FlaskClient, db_session):
    # Query the database
    table = db_session.query(Table).count()
    assert table > 0


def test_each_users_registration(client: FlaskClient, register_user_temp):
    pass


def test_all_users_registration(client: FlaskClient, register_user):
    pass


def test_users_log_in_success(client: FlaskClient, users_auth_header):
    pass


def test_teacher_dashboard(client: FlaskClient, teacher_auth_header):
    response = client.get(
...
def test_available_subjects_for_registration(
    client: FlaskClient, create_semester, stud_auth_header
):
    response = client.get(
...
def test_get_registered_grades(
    client: FlaskClient, admin_auth_header, stud_course_register
):
    response = client.get(
