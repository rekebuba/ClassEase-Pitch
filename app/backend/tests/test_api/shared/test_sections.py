...
class TestSectionsApi:
    ...
    def test_get_sections(
        ...
        if expected_status == 200:
            expected_fields = (
                fields if fields is not None else SectionSchema.default_fields()
            )
    ...
    def test_get_section_by_id(
        ...
        url = f"/api/v1/years/{academic_year.id}/sections/{section_id}"
        if fields:
            url += f"?fields={','.join(fields)}"
    ...
    def test_get_section_by_id_not_found(
        ...
        url = f"/api/v1/years/{academic_year.id}/sections/{non_existent_section_id}"
        response = client.get(url, headers=random_auth_header["header"])
