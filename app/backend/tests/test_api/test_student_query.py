...
def student_data(db_session: scoped_session[Session]) -> Iterator[Student]:
    ...
    for grade_id in grade_ids:
        ...
        StudentFactory.create_batch(1, current_grade_id=grade_id[0])
...
class TestAdminStudentQueries:
    @pytest.mark.parametrize(
        ...
        [
            ...
            ([], 12),  # No grade filter should return all
        ],
    )
    def test_grade_filtering_when_no_one_registered(
        ...
        print(table_resp.json["tableId"]["studentName"])
        print(type(table_resp.json["tableId"]["studentName"]))
