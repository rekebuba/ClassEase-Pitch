...
def ceo_required(
    ...
    """
    Decorator to ensure that the request is made by an authenticated CEO.
    ...
    Raises:
        ...
        404 Not Found: If the token is valid but the CEO does not exist.
    """
    ...
    def decorated_function(*args: Any, **kwargs: Any) -> Union[T, Tuple[Response, int]]:
        ...
        try:
            payload = jwt.decode(
                token, current_app.config["CEO_SECRET_KEY"], algorithms=["HS256"]
            )
            ceo_data = storage.get_first(CEO, username=payload.get("username"))
            if ceo_data:
                return f(ceo_data, *args, **kwargs)
        except jwt.ExpiredSignatureError:
            return jsonify({"message": "Token has expired"}), 401
        except jwt.InvalidTokenError:
