...
function StudentScorePanel({ yearlyScore, isAssesOpen }) {
  return (
        ...
        <CardContent className="p-4">
          <div className="flex justify-between items-center mb-3">
            <h2 className="text-lg font-semibold">
              ...
              <span className="bg-green-500 text-white px-2 py-1 text-sm rounded-lg">
                Validated
              </span>
            </h2>
          </div>
          ...
          <div className="mt-3 space-y-2">
            {yearlyScore
              ...
              && Object.entries(yearlyScore).map(([key, item], index) => (
                <div
                  key={key}
                  className={`flex justify-between border-b-2 border-gray-100 pb-2 ${index === 0 ? "border-t-2" : ""}`}
                >
                  <p>
                    Grade
                    {item.grade}
