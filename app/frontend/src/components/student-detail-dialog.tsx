...
interface StudentDetailDialogProps {
    ...
    isOpen: boolean
    onClose: () => void
    onStatusChange: (studentId: string, newStatus: StudentApplication["status"]) => void
}

export default function StudentDetailDialog({ student, isOpen, onClose, onStatusChange }: StudentDetailDialogProps) {
    const [isUpdating, setIsUpdating] = useState(false)

    if (!student) return null

    const handleStatusChange = async (newStatus: StudentApplication["status"]) => {
        setIsUpdating(true)
        // Simulate API call
                        ...
                        <Card>
                            ...
                            <CardContent>
                                <div className="flex flex-wrap gap-2">
                                    <Button
                                        size="sm"
                                        variant={student.status === "under-review" ? "default" : "outline"}
                                    ...
                                    <Button
                                        size="sm"
                                        variant={student.status === "documents-required" ? "default" : "outline"}
                                    ...
                                    <Button
                                        size="sm"
                                        variant={student.status === "rejected" ? "destructive" : "outline"}
                                    ...
                                    <Card>
                                        ...
                                        <CardContent className="space-y-3">
                                            ...
                                            {student.grandFatherName && (
                                                <div>
                                                    <p className="text-sm font-medium text-gray-500">Grandfather's Name</p>
                                                ...
                                                {student.guardianName && (
                                                    <div>
                                                        ...
                                                        {student.guardianRelation && (
                                                            <p className="text-sm text-gray-600">Relationship: {student.guardianRelation}</p>
                                                        )}
                                                        {student.guardianPhone && <p className="text-sm text-gray-600">{student.guardianPhone}</p>}
                                                    </div>
                                                )}
                                                {student.emergencyContactName && (
                                                    <div>
                                                        ...
                                                        {student.emergencyContactPhone && (
                                                            <p className="text-sm text-gray-600">{student.emergencyContactPhone}</p>
                                                        )}
                                                    </div>
                                                )}
                                            </div>
                            ...
                            <TabsContent value="academic" className="space-y-4">
                                <Card>
                                    ...
                                    <CardContent className="space-y-4">
                                        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                                            ...
                                            <div>
                                                <p className="text-sm font-medium text-gray-500">Transfer Student</p>
                                                {student.isTransfer ? (
                                        ...
                                        {student.isTransfer && student.previousSchool && (
                                            <div className="p-4 bg-orange-50 rounded-lg">
                                                <p className="text-sm font-medium text-orange-800 mb-2">Transfer Information</p>
                                                <p className="text-sm">
                                                    <strong>Previous School:</strong> {student.previousSchool}
                                                </p>
                                                {student.previousGrades && (
                                                    <p className="text-sm mt-1">
                                                        <strong>Previous Performance:</strong> {student.previousGrades}
                                                    </p>
                                                )}
                                            </div>
                                        )}
                                            ...
                                            {student.transportation && (
                                                <div className="flex items-center gap-2">
                                                    ...
                                                    <div>
                                                        ...
                                                        <p className="capitalize">{student.transportation.replace("-", " ")}</p>
                                                    </div>
                                                </div>
                                            )}
                                        </div>
                                ...
                                {student.siblingInSchool && student.siblingDetails && (
                                    <Card>
                                        ...
                                        <CardContent>
                                            <p className="whitespace-pre-wrap">{student.siblingDetails}</p>
                                        </CardContent>
                                    </Card>
                                )}
                            </TabsContent>
                                        ...
                                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                            <div className="space-y-3">
                                                ...
                                                <div>
                                                    ...
                                                    <p className="text-sm text-gray-600">{student.fatherPhone}</p>
                                                </div>
                                                <div>
                                                    <p className="text-sm font-medium text-gray-500">Mother's Phone</p>
                                            ...
                                            <div className="space-y-3">
                                                <h4 className="font-medium text-orange-600">Additional Contacts</h4>
                                                {student.guardianName ? (
                                                    <div>
                                                        <p className="text-sm font-medium text-gray-500">Guardian</p>
                                                        <p>{student.guardianName}</p>
                                                        {student.guardianRelation && (
                                                            <p className="text-sm text-gray-600">({student.guardianRelation})</p>
                                                        )}
                                                        {student.guardianPhone && <p className="text-sm text-gray-600">{student.guardianPhone}</p>}
                                                    </div>
                                                ) : (
                                                    <p className="text-sm text-gray-500">No guardian specified</p>
                                                )}
                                                ...
                                                {student.emergencyContactName ? (
                                                    ...
                                                    <p className="text-sm text-gray-500">No emergency contact specified</p>
                                                )}
                                            </div>
                                        </div>
                                    </CardContent>
                                ...
                                <Card>
                                    ...
                                    <CardContent className="space-y-4">
                                        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                                            ...
                                            <div className="flex items-center gap-2">
                                                <p className="text-sm font-medium text-gray-500">Disabilities:</p>
                                        ...
                                        {student.medicalDetails && (
                                            <div className="p-4 bg-red-50 rounded-lg">
                                                <p className="text-sm font-medium text-red-800 mb-2">Medical Condition Details:</p>
                                                <p className="text-sm text-red-700">{student.medicalDetails}</p>
                                            </div>
                                        )}

                                        {student.disabilityDetails && (
                                            <div className="p-4 bg-orange-50 rounded-lg">
                                                <p className="text-sm font-medium text-orange-800 mb-2">Disability Details:</p>
                                                <p className="text-sm text-orange-700">{student.disabilityDetails}</p>
                                            </div>
                                        )}
                                        ...
                                        {!student.hasMedicalCondition &&
                                            !student.hasDisability && (
                                                <div className="p-4 bg-green-50 rounded-lg text-center">
                                                    ...
                                                    <p className="text-sm text-green-800">No medical concerns or special needs reported</p>
                                                </div>
                                            )}
                                    </CardContent>
                                </Card>
                                ...
                                <Card>
                                    ...
                                    <CardContent>
                                        <div className="bg-gray-50 p-4 rounded-lg space-y-2">
                                            <p>
                                                <strong>Student:</strong> {student.firstName} {student.fatherName}
                                            </p>
                                            <p>
                                            ...
                                            <p>
                                                <strong>Registration Date:</strong> {formatDate(student.registrationDate)}
                                            </p>
                                            {student.isTransfer && (
                                                <p>
                                                    <strong>Transfer Student:</strong> Yes (from {student.previousSchool})
                                                </p>
                                            )}
                                            {student.hasMedicalCondition && (
                                                <p className="text-red-600">
                                                    <strong>⚠️ Medical Condition:</strong> Requires attention
                                                </p>
                                            )}
                                            {student.hasDisability && (
