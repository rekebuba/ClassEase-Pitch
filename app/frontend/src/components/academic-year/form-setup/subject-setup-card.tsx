...
type Grade =
  YearSetupType["subjects"][number]["grades"] extends Array<infer G>
    ? G
    : never;
type Subject = YearSetupType["subjects"][number];
type Stream = YearSetupType["subjects"][number]["streams"][number];

export default function SubjectSetupCard({
  open,
  onOpenChange,
  mode,
  formIndex,
}: SubjectSetupCardProps) {
  const {
    control,
    watch: parentWatch,
    setValue,
  ...
  const syncGradesFromSubjects = useCallback(() => {
    ...
    const updatedGrades = grades.map((grade) => {
      const hasThisSubjectInGrade = subject.grades?.some(
        (g) => g.id === grade.id,
      );
      ...
      const updatedStreams = (grade.streams || []).map((stream) => {
        const hasThisSubjectInStream = subject.streams?.some(
          (st) => st.id === stream.id,
        );
    ...
    <Dialog open={open} onOpenChange={handleCancel}>
      <DialogContent className="max-w-5xl flex flex-col max-h-[90vh]">
        <DialogHeader className="flex items-center justify-between border-b pb-4 text-lg font-semibold">
          ...
          <DialogDescription>
            {watchSubForm?.name ? "Configure" : "Create New"} Subject.
          </DialogDescription>
        </DialogHeader>
                    ...
                    <CardContent className="p-6">
                      ...
                      {watchParentForm.grades.length === 0 && (
                        <div className="text-center py-12 text-gray-500">
                          ...
                          <p>
                            No grades added yet. Add your first grade in the
                            'Grades & Streams' Tab
                          </p>
                        </div>
                      )}
