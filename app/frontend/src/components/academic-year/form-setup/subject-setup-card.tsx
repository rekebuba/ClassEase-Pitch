...
export default function SubjectSetupCard({ open, onOpenChange, mode, formIndex, onDirty }: SubjectSetupCardProps) {
    ...
    const { fields: subjectFields, remove: removeSubject } = useFieldArray({ control: control, name: "subjects" })
    ...
    useEffect(() => {
        if (open && mode === "edit") {
            const subjectData = getValues().subjects[formIndex];
            if (subjectData) {
    ...
    const handleCancel = () => {
        if (mode === "create") {
            ...
            if (defaultData) {
                setValue(`subjects.${formIndex}`, defaultData);
            }
        }
        onOpenChange(false);
    };
