...
type Subject =
  YearSetupType["grades"][number]["subjects"] extends Array<infer G>
    ? G
    : never;
    ...
    <Dialog open={open} onOpenChange={handleCancel}>
      <DialogContent className="max-w-5xl flex flex-col max-h-[90vh]">
        <DialogHeader className="flex items-center justify-between border-b pb-4 text-lg font-semibold">
          ...
          <DialogDescription>
            {watchForm.grades?.[formIndex]?.grade ? "Configure" : "Create New"}{" "}
            grade level and academic tracks.
          </DialogDescription>
        </DialogHeader>
            ...
            <CardContent className="space-y-6 pt-4 border-t">
              ...
              <div>
                ...
                <p className="text-sm text-gray-500 mb-3">
                  Selected subjects for this grade
                </p>
                  ...
                  {watchForm.subjects.map((subject, subjectIndex) => (
                    ...
                    >
                      <CheckboxWithLabel<Subject>
                        ...
                        value={
                          watchForm.grades[formIndex].subjects.find(
  ...
  useEffect(() => {
    ...
    streamQueries.forEach((query, index) => {
      if (query.isSuccess && query.data && index < streamFields.length) {
        ...
        form.setValue(
          `grades.${formIndex}.streams.${index}.subjects`,
          subjects,
        );
      }
    });
      ...
      <div className="flex flex-wrap gap-2">
        {["Natural Science", "Social Science"].map((stream, streamIndex) => (
          <Button
            ...
            disabled={watchForm.grades[formIndex].streams.some(
              (s) => s.name === stream,
            )}
          >
        ...
        <Dialog open={isOpen} onOpenChange={setIsOpen}>
          <DialogTrigger asChild>
            <Button
              ...
              className="text-sm"
              onClick={() =>
                prependStream({ id: "", gradeId: "", name: "", subjects: [] })
              }
            >
              <Plus className="h-4 w-4 mr-1" />
              Custom stream name
                ...
                .map((subject, subjectIndex) => (
                  <div key={subject.id} className="flex items-center space-x-2">
                    <CheckboxWithLabel<Subject>
                      ...
                      value={
                        watchForm.grades[formIndex].streams[
                          streamIndex
