...
export function DataTableInputFilter<TData, TValue>({
  ...
  const filterValue =
    textFilterValue ?? (type === "range" || type === "number" ? 0 : "");
  ...
  const selectedOperator =
    columnFilter?.operator || comparisonOperators[0].value;

  const defaultRange = column.columnDef.meta?.range;
  const unit = column.columnDef.meta?.unit;

  const handleValueChange = React.useCallback(
    (value: String | Number, operator: String) => {
      if (
        defaultRange?.max &&
    ...
    <>
      {isAdvancedMode && (
        <Select
          ...
          onValueChange={(operator) => handleValueChange(filterValue, operator)}
        >
