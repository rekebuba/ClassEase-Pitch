...
export function DataTableFilterList<TData>({
  ...
  const [joinOperator, setJoinOperatorState] = React.useState<JoinOperator>("and")

  // Load initial filters and join operator from URL
  React.useEffect(() => {
    ...
    const joinOperatorParam = searchParams.get(JOIN_OPERATOR_KEY) as JoinOperator
    if (joinOperatorParam && (joinOperatorParam === "and" || joinOperatorParam === "or")) {
      setJoinOperatorState(joinOperatorParam)
    }
  }, [searchParams])

  // Debounce function for updating URL
  const updateSearchParams = React.useCallback(
    (newFilters: ExtendedColumnFilter<TData>[], newJoinOperator: JoinOperator) => {
      const newParams = new URLSearchParams(searchParams)

      if (newFilters.length > 0) {
        newParams.set(FILTERS_KEY, JSON.stringify(newFilters))
      } else {
        newParams.delete(FILTERS_KEY)
      }

      newParams.set(JOIN_OPERATOR_KEY, newJoinOperator)
  ...
  const debouncedUpdateSearchParams = React.useMemo(() => {
    let timeoutId: NodeJS.Timeout | null = null

    return (newFilters: ExtendedColumnFilter<TData>[], newJoinOperator: JoinOperator) => {
      if (timeoutId) {
        clearTimeout(timeoutId)
      }

      timeoutId = setTimeout(() => {
        updateSearchParams(newFilters, newJoinOperator)
        timeoutId = null
      }, debounceMs)
    }
  ...
  const setFilters = React.useCallback(
    (newFilters: ExtendedColumnFilter<TData>[]) => {
      ...
      debouncedUpdateSearchParams(newFilters, joinOperator)
    },
    [debouncedUpdateSearchParams, joinOperator],
  )

  // Set join operator
  const setJoinOperator = React.useCallback(
    (newJoinOperator: JoinOperator) => {
      setJoinOperatorState(newJoinOperator)
      debouncedUpdateSearchParams(filters, newJoinOperator)
    },
    [debouncedUpdateSearchParams, filters],
  )
