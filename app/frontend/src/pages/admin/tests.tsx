...
import type { Grade, Subject } from "@/lib/academic-year";
...
export default function AcademicYearSetup({
  ...
  const [academicYear, setAcademicYear] =
    useState<Partial<DetailAcademicYear>>(initialData);
  ...
  const addGrade = (gradeSuggestion?: (typeof GRADE_SUGGESTIONS)[0]) => {
    console.log("Adding grade with suggestion:", gradeSuggestion);
        ...
        <Card>
          <CardContent className="p-4">
            <div className="flex items-center justify-between text-sm">
              ...
              <div className="flex items-center gap-2">
                <div
                  className={`w-3 h-3 rounded-full ${validateForm() ? "bg-green-500" : "bg-gray-300"}`}
                />
                <span>Complete</span>
              </div>
            </div>
              ...
              <CardContent className="space-y-6">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  ...
                  <div>
                    ...
                    <Input
                      ...
                      onChange={(e) =>
                        updateAcademicYear({ endDate: e.target.value })
                      }
                    />
                  </div>
                </div>
                ...
                <div>
                  ...
                  <Input
                    ...
                    onChange={(e) =>
                      updateAcademicYear({ name: e.target.value })
                    }
                ...
                <div>
                  ...
                  <Select
                    ...
                    onValueChange={(value: "Semester" | "Quarter") =>
                      updateAcademicYear({ calendarType: value })
                    }
                  >
                    <SelectTrigger>
                      <SelectValue />
              ...
              <CardContent>
                ...
                <div className="mb-6">
                  ...
                  <p className="text-sm text-gray-600 mb-3">
                    Add common grade levels with pre-configured subjects
                  </p>
                  <div className="flex flex-wrap gap-2">
                    {GRADE_SUGGESTIONS.map((grade) => (
                      <Button
                        ...
                        disabled={academicYear.grades?.some(
                          (g) => g.grade === grade.name,
                        )}
                      >
                  ...
                  {academicYear.grades?.map((grade) => (
                    <GradeSetupCard
                      ...
                      onUpdate={(updatedGrade) =>
                        updateGrade(grade.id, updatedGrade)
                      }
                      onRemove={() => removeGrade(grade.id)}
                      availableSubjects={
                        academicYear.subjects?.map((s) => s.name) || []
                      }
                    />
                  ))}
                </div>
                ...
                {academicYear.grades?.length === 0 && (
                  <div className="text-center py-12 text-gray-500">
                    ...
                    <p>
                      No grades configured yet. Add your first grade level
                      above.
                    </p>
                  </div>
                )}
          ...
          <TabsContent value="review" className="space-y-6">
            <Card>
              <CardHeader>
                ...
                <p className="text-sm text-gray-600">
                  Review your academic year configuration before finalizing
                </p>
              </CardHeader>
              <CardContent className="space-y-6">
                {/* Basic Info Summary */}
                ...
                <div>
                  <h4 className="font-medium mb-3">
                    Grades & Streams ({academicYear.grades?.length || 0})
                  </h4>
                ...
                <div>
                  <h4 className="font-medium mb-3">
                    Subjects ({academicYear.subjects?.length || 0})
                  </h4>
                  <div className="flex flex-wrap gap-2">
                ...
                {/* Validation Status */}
                <Card
                  className={`${validateForm() ? "bg-green-50 border-green-200" : "bg-red-50 border-red-200"}`}
                >
                  <CardContent className="p-4">
                    <h4 className="font-medium mb-2">
                      {validateForm()
                    ...
                    <div className="text-sm">
                      {validateForm() ? (
                        <p>
                          Your academic year is fully configured and ready to be
                          saved.
                        </p>
                      ) : (
                        <div>
                          ...
                          <ul className="list-disc list-inside space-y-1">
                            {!academicYear.name && (
                              <li>Set academic year name</li>
                            )}
                            {!academicYear.startDate && <li>Set start date</li>}
                            {!academicYear.endDate && <li>Set end date</li>}
                            {!academicYear.grades?.length && (
                              <li>Add at least one grade level</li>
                            )}
                            {!academicYear.subjects?.length && (
                              <li>Add at least one subject</li>
                            )}
                          </ul>
                        </div>
                      )}
