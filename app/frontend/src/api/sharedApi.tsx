...
const sharedApi = {
    ...
    return zodApiHandler(
      () =>
        api.get(`/users/${userId}`, {
          params: buildQueryParams(schema, params),
        }),
      schema,
    );
  },
  getYear: <T extends z.ZodTypeAny>(
    ...
    return zodApiHandler(
      () => api.get("/years", { params: buildQueryParams(schema, params) }),
      schema,
    );
  },
  ...
  getAcademicYears: async () =>
    zodApiHandler(() => api.get("/academic_years"), YearSchema.array()),
  getSubjects: async () =>
    zodApiHandler(() => api.get("/subjects"), SubjectSchema.array()),
  getGrades: async () =>
    zodApiHandler(() => api.get("/grades"), GradeSchema.array()),
  getStreams: async () =>
    zodApiHandler(() => api.get("/streams"), StreamSchema.array()),
  getStudentAssessment: (requirements) =>
    api.get("/student/assessment", { params: requirements }),
  getStudentAssessmentDetail: (requirements) =>
    api.get("/student/assessment/detail", { params: requirements }),
  updateProfile: (data) =>
  ...
  saveTeacherRegistration: (data: TeacherRegistrationFormData) =>
    api.post("/register/teacher", data),
};
...
export const saveTeacherRegistration = async (
  data: TeacherRegistrationFormData,
) => {
  const response = await zodApiHandler(
