...
function GuestProfilePage() {
  const { profile, isProfileComplete, saveProfile, loadDemoProfile } = useGuest();
    ...
    <div className="mx-auto max-w-5xl space-y-6">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
        <div>
          ...
          <p className="text-muted-foreground">Manage the reusable profile submitted with your school applications.</p>
        </div>
        {isProfileComplete && (
          <Button asChild>
        ...
        <Card className="rounded-lg border-emerald-200 bg-emerald-50">
          <CardContent className="flex items-start gap-3 text-emerald-800">
            ...
            <div>
              ...
              <p className="text-sm">You're ready to apply to schools.</p>
            </div>
          </CardContent>
        </Card>
      )}
      <ProfileForm profile={profile} onSave={saveProfile} onUseSample={!isProfileComplete ? loadDemoProfile : undefined} />
    </div>
  );
}
