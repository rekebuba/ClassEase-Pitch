...
export const studentRegistrationSchema = z
  .object({
    ...
    languageAtHome: z
      ...
      .regex(
        /^[a-zA-Z\s,]+$/,
        "Language can only contain letters, spaces, and commas",
      )
      .optional(),
    transportation: z
      .enum(["school-bus", "parent-drop", "walk", "bicycle", "other"])
      .optional(),
    ...
    siblingDetails: z
      .string()
      .max(300, "Sibling details must be less than 300 characters")
      .optional(),
  })
  .refine(
    (data) => {
  ...
  .refine(
    (data) => {
      if (
        ...
        (!data.medicalDetails || data.medicalDetails.trim().length === 0)
      ) {
        return false;
      }
      return true;
    ...
    {
      message:
        "Medical condition details are required when medical condition is selected",
      path: ["medicalDetails"],
    },
  )
  .refine(
    (data) => {
      if (
        data.hasDisability &&
        (!data.disabilityDetails || data.disabilityDetails.trim().length === 0)
      ) {
        return false;
      }
      return true;
  ...
  .refine(
    (data) => {
      if (
        data.requiresAccommodation &&
        (!data.accommodationDetails ||
          data.accommodationDetails.trim().length === 0)
      ) {
        return false;
      }
      return true;
    ...
    {
      message:
        "Accommodation details are required when special accommodation is selected",
  ...
  .refine(
    (data) => {
      if (
        data.siblingInSchool &&
        (!data.siblingDetails || data.siblingDetails.trim().length === 0)
      ) {
        return false;
      }
      return true;
    ...
    {
      message:
        "Sibling details are required when sibling in school is selected",
      path: ["siblingDetails"],
    },
  );
...
export const teacherRegistrationSchema = z
  .object({
    ...
    // Emergency Contact
    emergencyContactName: nameSchema.min(
      1,
    ...
    subjectsToTeach: z
      .array(z.string())
      .min(1, "Please select at least one subject to teach"),
    gradeLevelsToTeach: z
      .array(z.string())
    ...
    reference1Organization: z
      .string()
      .min(1, "First reference organization is required"),
  ...
  .refine(
    ...
    {
      message:
        "License expiration date is required when you have a teaching license",
  ...
  .refine(
    (data) => {
      if (
        data.hasConvictions &&
        (!data.convictionDetails || data.convictionDetails.trim().length === 0)
      ) {
        return false;
      }
      return true;
    ...
    {
      message:
        "Conviction details are required when you have criminal convictions",
  ...
  .refine(
    (data) => {
      if (
        data.hasDisciplinaryActions &&
        (!data.disciplinaryDetails ||
          data.disciplinaryDetails.trim().length === 0)
      ) {
        return false;
      }
      return true;
...
export type TeacherRegistrationFormData = z.infer<
