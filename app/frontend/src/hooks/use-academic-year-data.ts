...
import { GradeSchema, StreamSchema, SubjectSchema, YearSchema } from "@/lib/api-response-type";
...
export function useAcademicYears() {
  ...
  const fetchAllData = async () => {
    ...
    try {
      const [subjectsRes, gradesRes, academicYearRes, streamsRes] = await Promise.all([
      ...
      if (!academicYearRes.success) newErrors.global = academicYearRes.error.message;
