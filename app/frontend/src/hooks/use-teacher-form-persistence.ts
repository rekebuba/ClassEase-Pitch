...
export function useTeacherFormPersistence() {
    ...
    const saveFormData = useCallback((data: TeacherRegistrationFormData, currentStep: number) => {
    ...
    const loadFormData = useCallback((): { data: Partial<TeacherRegistrationFormData>; step: number } | null => {
        try {
